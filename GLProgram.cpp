...
GLShader::GLShader( GLenum type, std::string const &source ) {
	...
	if (compile_status != GL_TRUE) {
		...
		GLsizei lengthi = 0;
		glGetShaderInfoLog(shader, (GLsizei)info_log.size(), &lengthi, &info_log[0]);
		std::cerr << "Info log: " << std::string(info_log.begin(), info_log.begin() + lengthi);
...
void GLProgram::DEBUG_dump_info(std::string const &_name) {
	std::cout << " ------ Shader Program '" << _name << "' -----\n";
	{
		GLint active = 0;
